...
websocket_url = f"wss://semantlyapi-352e1ba2b5fd.herokuapp.com/ws/{game_code}"
...
# Start WebSocket listener in asyncio event loop
def start_websocket_listener():
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(websocket_listener())

# Ensure the WebSocket listener runs in a separate asyncio loop
if 'websocket_thread' not in st.session_state:
    websocket_thread = threading.Thread(target=start_websocket_listener, daemon=True)
...
if player_name not in remaining_guesses or remaining_guesses[player_name] <= 0:
    ...
    if guess:
        ...
        if response.status_code == 200:
            score = response.json()["guess"]["score"]
